   ...
   public void getShortTermForecast(String area1, String area2) {
	 ...
	 try {
		 StringBuilder urlBuilder = new StringBuilder("http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst"); /*URL*/
		    urlBuilder.append("?" + URLEncoder.encode("serviceKey","UTF-8") + "=DsZ%2BkPrW0s6N%2BLEWaiT02cNs5pPUVlRixycZ61pVPpmHSiOwgJ4vwcw6T9YAjra8Hr0nVnQ06KqzpMikvedLYg%3D%3D"); /*Service Key*/
...
//		    System.out.println(sb.toString());
		    ...
		    //itemp 키에 해당하는 JSON 데이터는 여러 값이기 때문에 배열의 문법을 제공하는 객체로 받습니다. 
			JSONArray itemArray = (JSONArray) ((JSONObject) body.get("items")).get("item");
		    
			//item 내부의 각각의 객체에 대한 반복문을 작성
			for(Object obj :itemArray) {
				
				//형변화 진행.
				JSONObject item = (JSONObject) obj;
				//"category"키에 해당하는 단일 값 가져옵니다.
				String category = (String) item.get("category");
				//"fcstValue" 키에 해당하는 단일 값 가져옴
