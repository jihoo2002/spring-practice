...
public class MailSenderService {
	...
	public String joinEmail(String email) {
		...
		String setFrom = "yolohoo@naver.com"; //email-config에 설정한 발신용 이메일 주소 입력
	...
	//	mailSend(setFrom, toMail, title, content); //메서드를 부르면 실제로 메일 전송됨!
		
		return Integer.toString(authNum);
		//컨트롤러에게 인증번호(난수)를 문자열로 바꾸어 리턴한다. 
	...
	private void mailSend(String setFrom, String toMail, String title, String content) {
		...
		try {
			...
			helper.setFrom(setFrom); //발신
			helper.setTo(toMail); //수신
