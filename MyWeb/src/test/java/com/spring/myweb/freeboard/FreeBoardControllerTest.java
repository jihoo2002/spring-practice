...
@WebAppConfiguration //어플리케이션 컨텍스트의 웹 버전을 사용하는 아노테이션 
public class FreeBoardControllerTest {
    ...
    */
	...
	@Autowired //컴퓨터에 있는 모든 빈들을 테스트 할떄
	private WebApplicationContext ctx; //가상 환경 세팅을 위한 객체 생성
	
	//@Autowired
	//private FreeBoardController controller; ->이 컨트롤러 하나만 테스트 할때
	...
	public void setup() {
		...
		//테스트할 컨트롤러를 수동으로 주입 하나의 컨트롤러 자체만 테스트를 진행 할때는 이렇게 씁니다.
	...
	@DisplayName("/freeboard/freeList 요청 처리과정 테스트")
		...
		ModelAndView mv =mockMvc.perform(MockMvcRequestBuilders.get("/freeboard/freeList"))  //사용자 대신 가상요청을 보낸다.
			.andReturn() //요청의 결과를 받음
			.getModelAndView(); //요청 결과에서 ModelAndView 객체를 얻음.
			
			//컨트롤러에서 Model 객체를 담은 데이터를 확인.
	...
	@DisplayName("3번 글 상세 보기 요청을 넣으면"
			+ " 컨트롤러는 DB에서 가지고 온 글 객체를 model에 담아 freeDetail.jsp로 이동")
