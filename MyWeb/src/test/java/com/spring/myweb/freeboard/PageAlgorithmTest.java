...
public class PageAlgorithmTest {
	...
	void pageTest() {
		int page =16; //사용자가 요청한 페이지번호
		int cpp = 20; //한 화면에 보여줄 게시물 개수
		int btnNum = 5; //한 화면에 보여질 버튼 개수
		
		int acticleTotablCount = mapper.getTotal(Page.builder()
				.pageNo(16)
				.amount(20)
				.build());
		System.out.println("총 게시물 수: " +acticleTotablCount);
		
		//끝 페이지 번호 구하기
		int endPage = (int)(Math.ceil(page/(double)btnNum)*btnNum);
		
		System.out.println("보정 전 끝페이지 번호:" +endPage);
		
		
		//시작 페이지 번호 구하기
		int beginPage = endPage - btnNum + 1;
		System.out.println("시작 페이지번호: " +beginPage);
		
		boolean prev = (beginPage==1)? false: true;
		boolean next = (acticleTotablCount <= (endPage*cpp))? false:true;
		
		System.out.println("이전 버튼 활성화: " +prev);
		System.out.println("다음 버튼 활성화: " +next);
	
		if(!next) { //false라면
			endPage = (int)Math.ceil(acticleTotablCount / (double)cpp);
		}
